...
class Spreadsheet_Excel_Reader {
	...
	function _parse() {
		...
		$code = v($data,$pos);
		$length = v($data,$pos+2);
                $result = \PMVC\plug('excel')->xls_parse(
                  $version,
                  $data,
                  $code,
                  $pos,
                ...
                $this->sst = array_merge($this->sst, $result['sst']);
                $this->fontRecords = array_merge($this->fontRecords, $result['fontRecords']);
                $this->colors =  array_merge($this->colors, $result['colors']);
                $this->xfRecords = array_merge($this->xfRecords, $result['xfRecords']);
                $this->nineteenFour = $result['nineteenFour'];
                $this->boundsheets = $result['boundsheets'];
		foreach ($result['boundsheets'] as $key=>$val){
                  $this->sn = $key;
		...
		while($cont) {
			...
			switch ($code) {
				...
				case SPREADSHEET_EXCEL_READER_TYPE_LABELSST:
					...
					$this->addcell($row, $column, \PMVC\get($this->sst, $index), array('xfIndex'=>$xfindex) );
					break;
	...
	function _encodeUTF16($string) {
		return \PMVC\plug('utf8')->decodeUtf16($string);
	}
